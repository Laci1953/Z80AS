...
;extern
	global	Z80ASM,HOFNAM,HOFDAT,HOFTIM,LFLAG,OFLAG,QFLAG
	global	ERRFLG,CMPHD,MAXMEM,SYMTBL,VALID,MALLOC,MFREE
	global	FNDREC,CNV2HX,NAMLEN,ZERODS,DEFCPU,RADIX,UMODE
	global	CMNPTR
...
;
GETOPT:	ld	bc,0
	ld	(LSTFILE),bc	;init file name pointers
	ld	(OBJFILE),bc
	xor	a
	ld	(QFLAG),a
next:
...
2:
	...
	ld	a,0FFH
	ld	(QFLAG),a	;but set "brief" mode
	inc	hl
	jr	next
3:
	cp	'J'		;ignore J option
	jr	nz,4f
	inc	hl
	jr	next
4:
...
ARGSMSG:defm	'Usage: Z80AS [-L[listfile]] [-Oobjfile] sourcefile'
	defb	0
